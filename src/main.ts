...
var leftLane = Bodies.fromVertices(80,700,[[{x:0,y:0},{x:0,y:110},{x:90,y:130}]], {isStatic: true})
var rightLane = Bodies.fromVertices(460,700,[[{x:0,y:0},{x:0,y:110},{x:-90,y:130}]], {isStatic: true})

var leftEdge: Body  = Bodies.rectangle(0, 550, 20, 700, { isStatic: true });
var rightEdge: Body  = Bodies.rectangle(600, 550, 20, 700, { isStatic: true });
...
var leftRamp: Body  = Bodies.rectangle(90, 800, 160, 20, { isStatic: true,  angle: Math.PI * .07});
var rightRamp: Body  = Bodies.rectangle(470, 800, 160, 20, { isStatic: true,  angle: Math.PI * -.07 });
...
var leftFlipper: Body = Bodies.fromVertices(200, 824, [[{x:0,y:0},{x:80, y:20},{x:80,y:24},{x:0,y:24}]],{isStatic:true})
var rightFlipper: Body = Bodies.fromVertices(360, 824, [[{x:0,y:0},{x:-80, y:20},{x:-80,y:24},{x:0,y:24}]],{isStatic:true,})
...
var ball: Matter.Body = Bodies.circle(572, 700, 15, {restitution:.3  })

/* ----------------------------------------------------- */

// add all of the bodies to the world
Composite.add(engine.world, [spring, leftRamp, rightRamp, arch, leftFlipper, 
    rightFlipper,leftBlock,rightBlock,leftLane,rightLane,
  leftEdge, rightEdge, laneBoundry, oob, bouncer1, bouncer2, bouncer3, ball]);
...
Events.on(engine, 'collisionStart', function(event) {
    for (var i = 0; i < event.pairs.length; i++) {
        ...
        if (bodyB == ball && bodyA.label == 'bouncer') {
            launchBall(ball, bodyA.position, .04)
        }
        else if (bodyA == ball && bodyB.label== 'bouncer') {
            launchBall(ball, bodyB.position, .04)
        }
